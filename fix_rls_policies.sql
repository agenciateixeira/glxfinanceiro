...
CREATE POLICY "Users can view spouse bank accounts"
  ...
  USING (
    EXISTS (
      ...
      WHERE (shared_accounts.user_id = auth.uid() AND shared_accounts.spouse_user_id = bank_accounts.user_id)
         OR (shared_accounts.spouse_user_id = auth.uid() AND shared_accounts.user_id = bank_accounts.user_id)
    )
  );
...
CREATE POLICY "Users can view spouse transfers"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM shared_accounts
      WHERE (shared_accounts.user_id = auth.uid() AND shared_accounts.spouse_user_id = account_transfers.user_id)
         OR (shared_accounts.spouse_user_id = auth.uid() AND shared_accounts.user_id = account_transfers.user_id)
    )
  );
